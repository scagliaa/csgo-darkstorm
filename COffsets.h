...
public:
	...
	void COffsets::Initialize()
	{
		...
		offset_DT_LocalPlayerExclusive__m_flLaggedMovementValue = iFindOffset(
			p->GetDataTable(), "m_flLaggedMovementValue");
...
//gBaseAPI.LogToFile( /*---[END INIT OFFSETS]---*/XorStr<0x98,25,0x029AAE28>("\xB5\xB4\xB7\xC0\xD9\xD3\xDA\xBF\xE9\xEF\xEB\xF7\x84\xEA\xE0\xE1\xFB\xEC\xFE\xF8\xF1\x80\x83\x82"+0x029AAE28).s );
	...
	CBaseEntity* COffsets::pGetBaseCombatWeapon(CBaseEntity * pEntity, int index)
	{
		int hMyWeapon = *MakePtr(int, pEntity, offset_DT_BaseCombatCharacter__m_hMyWeapons + index * 4);
		return static_cast<CBaseEntity*>(gInts.EntList->GetClientEntityFromHandle(hMyWeapon));
	}

	float COffsets::GetNextAttack(CBaseEntity * ent)
	{
		return *MakePtr(float, ent, offset_DT_BaseCombatWeapon__m_flNextPrimaryAttack);
	}

	int COffsets::GetTickBase(CBaseEntity * ent)
	{
		return *MakePtr(int, ent, offset_DT_LocalPlayerExclusive__m_nTickBase);
	}

	int COffsets::GetHealth(CBaseEntity * pEntity)
	{
		return *MakePtr(int, pEntity, offset_DT_BasePlayer__m_iHealth);
	}

	int COffsets::GetTeamNum(CBaseEntity * pEntity)
	{
		return *MakePtr(int, pEntity, offset_DT_BaseEntity__m_iTeamNum);
	}

	bool COffsets::IsAlive(CBaseEntity * pBaseEntity)
	{
		return LIFE_ALIVE == *MakePtr(BYTE, pBaseEntity, offset_DT_BasePlayer__m_lifeState);
	}

	int COffsets::GetFlags(CBaseEntity * pBaseEntity)
	{
		return *MakePtr(int, pBaseEntity, offset_DT_BasePlayer__m_fFlags);
	}

	Vector COffsets::m_vecPunchAngle(CBaseEntity * pBaseEntity)
	{
		return *MakePtr(Vector, pBaseEntity, offset_DT_LocalPlayerExclusive__m_vecPunchAngle);
	}

	CBaseEntity* COffsets::GetWeaponOwner(CBaseEntity * pWeapon)
	{
		int hOwner = *MakePtr(int, pWeapon, offset_DT_BaseCombatWeapon__m_hOwner);
		return static_cast<CBaseEntity*>(gInts.EntList->GetClientEntityFromHandle(hOwner));
	}

	int iCreateMoveOffset = 24, iPaintTraverseOffset = 41, iFrameStageNotifyOffset = 37, iViewSetupOffset = 18,
	    iSceneEndOffset = 9, iFireEventClientSideOffset = 9, iSetViewAnglesOffset = 19, iDoPostScreenEffectsOffset = 44,
	    iLockCursorOffset = 67;
};
